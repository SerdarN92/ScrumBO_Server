...
public class FXMLDefinitionOfDoneController implements Initializable {
	...
	private void handleButtonBack(ActionEvent event) throws Exception {
		data.clear();
		this.root = FXMLLoader.load(getClass().getResource("/scrumbo/de/gui/FXMLUserStoryEdit.fxml"));
	...
	private void handleButtonCreateDefinitionOfDone(ActionEvent event) throws Exception {
		try {
			FXMLLoader fxmlLoader = new FXMLLoader(
					getClass().getResource("/scrumbo/de/gui/FXMLDefinitionOfDoneCreate.fxml"));
		...
		tableViewDefinitionOfDone.setRowFactory(tv -> {
			...
			row.setOnMouseClicked(event -> {
				if (event.getClickCount() == 1 && (!row.isEmpty())) {
					...
					try {
						FXMLLoader fxmlLoader = new FXMLLoader(
								getClass().getResource("/scrumbo/de/gui/FXMLDefinitionOfDoneEdit.fxml"));
						Parent root1 = (Parent) fxmlLoader.load();
						Stage stage = new Stage();
						stage.initModality(Modality.APPLICATION_MODAL);
						stage.setScene(new Scene(root1));
