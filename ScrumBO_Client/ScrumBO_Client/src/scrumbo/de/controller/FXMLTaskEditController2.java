...
public class FXMLTaskEditController2 implements Initializable {
	...
	private void handleButtonAdd(ActionEvent event) throws Exception {
		FXMLSprintBacklogAddUserStoryController.selectedUserStoryTask.setBeschreibung(taskbeschreibung.getText());
		FXMLSprintBacklogAddUserStoryController.selectedUserStoryTask
				.setAufwandinstunden(Integer.parseInt(aufwandinstunden.getText()));
		FXMLSprintBacklogAddUserStoryController.selectedUserStoryTask.setBenutzer(currentBenutzer);
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		taskbeschreibung.setText(FXMLSprintBacklogAddUserStoryController.selectedUserStoryTask.getBeschreibung());
		aufwandinstunden.setText(
				FXMLSprintBacklogAddUserStoryController.selectedUserStoryTask.getAufwandinstunden().toString());
		Integer platz = 0;
		for (int i = 0; i < benutzerList.size(); i++) {
			if (benutzerList.get(i).getEmail()
					.equals(FXMLSprintBacklogAddUserStoryController.selectedUserStoryTask.getBenutzer().getEmail()))
				platz = i;
		}
